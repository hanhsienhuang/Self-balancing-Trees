...
public:
    ...
    Iterator erase(Iterator pos){
        if(this != pos.container){
            throw std::invalid_argument("Iterator of wrong Multiset");
        }
...
template<typename Key, template<typename, typename> typename Tree>
class Multiset<Key, Tree>::Iterator{
friend Multiset;
public:
    Iterator() = delete;

    Iterator(Multiset* container_, const typename Tree<Element, Compare>::Iterator& it_) noexcept
...
template<typename Key, template<typename, typename> typename Tree>
class Multiset<Key, Tree>::Compare{
public:
